...
export async function loader({request, context: {storefront}}: LoaderArgs) {
  ...
  const seoCollection = {
    ...
    title: 'Поиск',
    handle: 'search',
    descriptionHtml: 'Результаты поиска',
    description: 'Результаты поиска',
    seo: {
      title: 'Поиск',
      description: `Найдено ${products.nodes.length} для "${searchTerm}"`,
    },
    metafields: [],
    products,
          ...
          {noResults && (
            <Section padding="x">
              <Text className="opacity-50">
                Нет результатов, попробуйте что-нибудь другое.
              </Text>
            </Section>
          )}
          <Suspense>
